...
public class Graph {
        ...
        for(Stop from:stops){   // Iterate through all pairs of stops
            for(Stop to:stops){
                if(!from.equals(to)){// Avoid creating edges from a stop to itself
                    double distance = from.distanceTo(to);
                    String name = from.getName() + " " + to.getName();
                    if(!visitedEdges.contains(name) && distance<=walkingDistance){
    ...
    public void removeWalkingEdges() {
        ...
        for(Stop stop:stops){
            stop.deleteInEdgesOfType(Transport.WALKING);
            stop.deleteOutEdgesOfType(Transport.WALKING);
        }
    }
