...
public class Graph {
    ...
    public void printGraphData(){
        ...
        for (Stop stop : stops){
            ...
            //System.out.println("  "+stop.getInEdges().size()+"  in edges; "); 
    ...
    private void createAndConnectEdges() {
        ...
        for (Line line : lines) {
            ...
            for (int i = 0; i < line.getStops().size() - 1; i++) {
                ...
                from.addNeighbour(to); //added part3
                to.addNeighbour(from);
                
                
                Edge edge = new Edge(from, to, transpType, line, distance);
                //Edge edge1 = new Edge(to, from, transpType, line, distance);
                edges.add(edge);
                //edges.add(edge1);    
    ...
    public void recomputeWalkingEdges(double walkingDistance) {
        removeWalkingEdges();// Remove existing walking edges before recomputing
        
        
        for(Stop from:stops){   // Iterate through all pairs of stops
            for(Stop to:stops){
                if(!from.equals(to)){// Avoid creating edges from a stop to itself
                    double distance = from.distanceTo(to);
                    if(distance<=walkingDistance){
                        // Create edges in both directions between the stops
                        Edge edge1 = new Edge(from,to,Transport.WALKING,null,distance);
                        Edge edge2 = new Edge(to,from,Transport.WALKING,null,distance);
    ...
    public void removeWalkingEdges() {
        ...
        for (Edge walkingEdge : toDelete) {
            Stop fromStop = walkingEdge.fromStop();
            Stop toStop = walkingEdge.toStop();

            fromStop.removeNeighbour(toStop);
            toStop.removeNeighbour(fromStop);
        }
